...
# ┌─────────────────────────────────────────────────────────────────────────┐
# │ State is persisted to ${STACKS_CONFIG}/{stack}/state-status/ files      │
# │ This allows:                                                            │
...
# └─────────────────────────────────────────────────────────────────────────┘
#
# State File Format (${STACKS_CONFIG}/{stack}/state-status/{unit_name}.json):
# {
...
init_remote_state_tracking() {
    ...
    ILTERO_STATE_DIR="$(pwd)/${STACKS_CONFIG:?STACKS_CONFIG must be set}/${stack_name}/state-status"
    export ILTERO_STATE_DIR
...
write_unit_state_status() {
    ...
    # Write status file (jq -n avoids JSON injection from variable values)
    jq -n \
        --arg unit "${unit_name}" \
        --arg status "${status}" \
        --arg ts "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
        --arg reason "${reason}" \
        '{unit: $unit, status: $status, timestamp: $ts, reason: $reason}' \
        > "${status_file}"
