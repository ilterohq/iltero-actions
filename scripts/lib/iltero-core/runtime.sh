...
run_runtime_scan() {
    ...
    if ! scan_result=$(iltero scan runtime \
        ...
        emit_cloud_credentials_hint_if_needed "${scan_result}"
        return 2
    fi
...
detect_drift() {
    ...
    if [[ "${drift_count}" -gt 0 ]]; then
        log_warning "Drift detected: ${drift_count} resource(s) have drifted"
...
check_runtime_compliance() {
    ...
    if ! result=$(iltero scan runtime-compliance \
        ...
        emit_cloud_credentials_hint_if_needed "${result}"
        return 2
    fi
